import mongoose, { InferSchemaType, model } from "mongoose";
const { Schema } = mongoose;

export const OrderStatus = ["success", "failed"] as const;
export const FulfillmentStatus = ["pending", "processing", "shipped", "delivered", "cancelled"] as const;
export const ReturnStatus = ["none", "requested", "approved", "rejected"] as const;

const OrderSchema = new Schema(
  {
    userId:      { type: String, required: true },
    email:       { type: String, required: true },
    sellerEmail: { type: String, default: null },
    amount:      { type: Number, required: true },
    status: { type: String, required: true, enum: OrderStatus },
    fulfillmentStatus: {
      type: String,
      enum: FulfillmentStatus,
      default: "pending",
    },
    returnStatus: {
      type: String,
      enum: ReturnStatus,
      default: "none",
    },
    returnReason: { type: String, default: "" },
    products: {
      type: [
        {
          productId: { type: Number, default: null },
          name:      { type: String, required: true },
          quantity:  { type: Number, required: true },
          price:     { type: Number, required: true },
        },
      ],
      required: true,
    },
  },
  { timestamps: true }
);

export type OrderSchemaType = InferSchemaType<typeof OrderSchema>;

export const Order = model<OrderSchemaType>("Order", OrderSchema);
